...
// *********************************************************************
int shrLog(int iLogMode = LOGCONSOLE, int iErrNum = 0, const char* cFormatString = "", ...)
{
    ...
    if ((pFileStream0 == NULL) && (iLogMode & LOGFILE))
    {
        ...
        if (cLogFilePathAndName == NULL)
        {
            ...
            errno_t err;
            if ((err = fopen_s(&pFileStream0, cLogFilePathAndName, cFileMode)) != 0)
            {
    ...
    if ((pFileStream1 == NULL) && (iLogMode & LOGFILE) && (iLogMode & MASTER))
    {
            ...
            errno_t err;
            if ((err = fopen_s(&pFileStream1, MASTERLOGFILE, "a+")) != 0)
            {
                // if error on attempt to open, be sure the file is null or close it, then return negative error code
                if (pFileStream1)
                {
...
char* shrFindFilePath(const char* filename, const char* executable_path) 
{
    ...
    // typical search locations for companion files (e.g. sample input data, or JIT source files)
    const char* searchPath[] = {
        ...
        "./inc/",                                   // "/inc/" subdir
        "../",                                      // up 1 in tree 
        "../data/",                                 // up 1 in tree, then "/data/" 
        "../src/",                                  // up 1 in tree, then "/src/" 
        "../inc/",                                  // up 1 in tree, then "/inc/" 
        "../../",                                   // up 2 in tree 
        "../../data/",                              // up 2 in tree, then "/data/" 
        "../../src/",                               // up 2 in tree, then "/src/" 
        "../../inc/",                               // up 2 in tree, then "/inc/" 
        "../../../src/<executable_name>/",          // up 3 in tree, then "/src/<exe name>/" 
        "../../../src/<executable_name>/data/",     // up 3 in tree, then "/src/data/" 
        "../../../src/<executable_name>/src/",      // up 3 in tree, then "/src/src/" 
        "../../../src/<executable_name>/inc/",      // up 3 in tree, then "/src/inc/" 
        "../../../sandbox/<executable_name>/",      // up 3 in tree, then "/sandbox/<exe name>/"
        "../../../sandbox/<executable_name>/data/", // up 3 in tree, then "/sandbox/data/"
        "../../../sandbox/<executable_name>/src/",  // up 3 in tree, then "/sandbox/src/"
        "../../../sandbox/<executable_name>/inc/"   // up 3 in tree, then "/sandbox/inc/"
    };
...
////////////////////////////////////////////////////////////////////////////////
shrBOOL shrComparePPM( const char *src_file, const char *ref_file, 
...
//		return shrFALSE;
	...
	if (error_count == 0) 
	{ 
		if(verboseErrors) std::cerr << "TEST PASSED\n\n"; 
	} else 
	{
		if(verboseErrors) std::cerr << "TEST FAILED !!! "<<error_count<<" errors...\n\n";
	}
...
unsigned char* shrLoadRawFile(const char* filename, size_t size)
{
    ...
    #endif
        {
            shrLog(LOGBOTH, 0," Error opening file '%s' !!!\n", filename);
            return 0;
        }
    ...
    shrLog(LOGBOTH, 0, " Read '%s', %d bytes\n", filename, read);
