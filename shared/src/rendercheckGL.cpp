...
{
    allocateMemory(width, height, bpp, bUsePBO);
}
...
CheckRender::savePPM(  const char *zfilename, bool bInvert, void **ppReadBuf )
{
    if (zfilename != NULL) {
        ...
        printf(" > Saving PPM: <%s>\n", zfilename);
        if (ppReadBuf) {
		    shrSavePPM4ub(zfilename, *(unsigned char **)ppReadBuf, m_Width, m_Height);
        } else {
		    shrSavePPM4ub(zfilename, (unsigned char *)m_pImageData, m_Width, m_Height);
        }
    }
}
...
void CheckRender::dumpBin(void *data, unsigned int bytes, char *filename) 
{
    ...
    {
        printf(" CheckRender::dumpBin: Error writing to <%s>\n", filename);
    }
    fflush(fp);
    fclose(fp);
}
...
bool CheckRender::compareBin2BinFloat(const char *src_file, const char *ref_file, unsigned int nelements, const float epsilon, const float threshold)
{
    ...
    else 
    {
        ...
        if (src_fp && ref_fp) 
        {
            ...
            if (fread(src_buffer, sizeof(float), nelements, src_fp) != nelements)
            {
                printf("compareBin2Bin <float> unable to read from src_file: %s\n", src_file);
                error_count++;
            }
            else if (fread(ref_buffer, sizeof(float), nelements, ref_fp) != nelements)
            {
                printf("compareBin2Bin <float> unable to read from ref_file: %s\n", ref_file_path);
                error_count++;
            }
            else
            {
