		 ...
		 wgSize(workgroupSize)
{
    ...
    shrCheckErrorEX(cViterbi == NULL, false, NULL);
    cpProgram = clCreateProgramWithSource(cxGPUContext, 1, (const char **)&cViterbi, &szKernelLength, &err);
    err = clBuildProgram(cpProgram, 0, NULL, "-cl-mad-enable", NULL, NULL);
    if (err != CL_SUCCESS)
    {
        // write out standard error, Build Log and PTX, then cleanup and exit
        shrLog(LOGBOTH | ERRORMSG, (double)err, STDERROR);
    ...
    shrCheckErrorEX(err, CL_SUCCESS, NULL);
    size_t wgSize;
    err = clGetKernelWorkGroupInfo(ckViterbiOneStep, device, CL_KERNEL_WORK_GROUP_SIZE, sizeof(size_t), &wgSize, NULL);
    shrCheckErrorEX(err, CL_SUCCESS, NULL);
    if (wgSize == 64) smallBlock = true;
    ...
    // take szLmem to the maximum multiple of 256 number to nState but smaller than 1536
    szLmem = int(nState/256) * 256;
    // if smaller than 256, set it to be the closest 2^n number
    if (nState < 256) szLmem = pow(2.0f, (int)(log((float)nState)/log(2.0f)));
    // if nState too big, restrict szLmem since local memory has only 16 KB
    if (nState > 1536) szLmem = 1536;
...
size_t HMM::ViterbiOneStep(const int &obs, const int &iObs)
{
    ...
    if (nState >= 128)
    {
        globalWorkSize[0] = 256*128;
		if (smallBlock) 
			localWorkSize[0] = 64;
		else 
			localWorkSize[0] = wgSize;
    } else {
        localWorkSize[0] = pow(2.0f, (int)(log((float)nState)/log(2.0f)));
    ...
    err |= clSetKernelArg(ckViterbiOneStep, 5, sizeof(float)*szLmem, NULL);
    err |= clSetKernelArg(ckViterbiOneStep, 6, sizeof(int)*szLmem, NULL);
    err |= clSetKernelArg(ckViterbiOneStep, 7, sizeof(float)*szSeg, NULL);
    err |= clSetKernelArg(ckViterbiOneStep, 8, sizeof(int)*szSeg, NULL);
    err |= clSetKernelArg(ckViterbiOneStep, 9, sizeof(int), (void*)&nState);
    err |= clSetKernelArg(ckViterbiOneStep, 10, sizeof(int), (void*)&szLmem);
    err |= clSetKernelArg(ckViterbiOneStep, 11, sizeof(int), (void*)&obs);
    err |= clSetKernelArg(ckViterbiOneStep, 12, sizeof(int), (void*)&iObs);

    err |= clEnqueueNDRangeKernel(cqCommandQue, ckViterbiOneStep, 1, NULL, globalWorkSize, localWorkSize, 0, NULL, NULL);
    shrCheckErrorEX(err, CL_SUCCESS, NULL);
