...
int iGraphicsWinHeight = ((float)uiImageHeight / (float)uiImageWidth) * iGraphicsWinWidth;  // GL Window height
...
int main(int argc, const char** argv)
{
    ...
    ckMedian = new cl_kernel[uiNumDevices];
    ...
    for (cl_uint i = 0; i < uiNumDevices; i++)
    {
        ...
        shrCheckErrorEX(ciErrNum, CL_SUCCESS, pCleanup);
        shrLog(LOGBOTH, 0, "clSetKernelArg (0-4), Device %u...\n\n", i); 
    }
...
//*****************************************************************************
void MedianFilterGPU(cl_uint* uiInputImage, cl_uint* uiOutputImage)
{
...
void DisplayGL()
{
    ...
    if (bFilter)
    {
        ...
        {
            MedianFilterHost (uiInput, uiOutput, uiImageWidth, uiImageHeight);
        }
    ...
    if (iFrameCount++ > iFrameTrigger)
    {
        ...
        {
            #ifdef _WIN32
                sprintf_s(cTitle, 256, "%s RGB Median Filter ON | W: %u , H: %u | %i fps | Proc. t = %.4f s | %.3f Mpix/s", 
            ...
            #else
                sprintf(cTitle, "%s RGB Median Filter ON | W: %u , H: %u | %i fps | Proc. t = %.4f s | %.3f Mpix/s", 
	                        cProcessor[iProcFlag], uiImageWidth, uiImageHeight, 
                            iFramesPerSec, dProcessingTime, (1.0e-6 * uiImageWidth * uiImageHeight)/dProcessingTime);  
            #endif
        }
...
void TestNoGL()
{
    ...
    shrLog(LOGBOTH | MASTER, 0, "oclMedianFilter, Throughput = %.4f, Time = %.5f, Size = %u, NumDevsUsed = %u, Workgroup = %u\n", 
...
void Cleanup(int iExitCode)
{
    ...
    {
        shrLog(LOGBOTH | CLOSELOG, 0, "oclMedianFilter.exe Exiting...\n");
    }
    else 
    {
        shrLog(LOGBOTH | CLOSELOG, 0, "oclMedianFilter.exe Exiting...\nPress <Enter> to Quit\n");
