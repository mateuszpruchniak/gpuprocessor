...
BodySystemOpenCL::BodySystemOpenCL(int numBodies, cl_device_id dev, cl_context ctx, cl_command_queue cmdq, unsigned int p, unsigned int q, bool usePBO)
  ...
  m_q(q)
{
    ...
    if (CreateProgramAndKernel(ctx, &dev, "integrateBodies_noMT", &noMT_kernel)) 
    {
        exit(shrLog(LOGBOTH | CLOSELOG, -1.0, "CreateProgramAndKernel _noMT ", STDERROR)); 
    }

    // create multithreaded program and kernel
    shrLog(LOGBOTH, 0, "\nCreateProgramAndKernel _MT... ");
    if (CreateProgramAndKernel(ctx, &dev, "integrateBodies_MT", &MT_kernel)) 
    {
        exit(shrLog(LOGBOTH | CLOSELOG, -1.0, "CreateProgramAndKernel _MT ", STDERROR)); 
    }
...
void BodySystemOpenCL::_initialize(int numBodies)
{
    ...
    if (m_bUsePBO)
    {
        ...
        shrLog(LOGBOTH, 0, "Allocating Pixel Buffers\n"); 
        for (int i = 0; i < 2; ++i)
        {
            ...
            {
                shrLog(LOGBOTH, -1.0, "WARNING: Pixel Buffer Object allocation failed!\n"); 
            }
    ...
    {
        AllocateNBodyArrays(cxContext, m_dPos, m_numBodies);
        shrLog(LOGBOTH, 0, "\nAllocateNBodyArrays m_dPos\n"); 
    }
    AllocateNBodyArrays(cxContext, m_dVel, m_numBodies);
...
float* BodySystemOpenCL::getArray(BodyArray array)
{
    ...
    CopyArrayFromDevice(cqCommandQueue, hdata, ddata, pbo, m_numBodies);
    ...
    switch (array)
    {
        ...
        case BODYSYSTEM_POSITION:
        {
            if (m_bUsePBO)
            {
                ...
                if ((unsigned)size != 4 * (sizeof(float) * m_numBodies))
                {
                    shrLog(LOGBOTH, -1.0, "WARNING: Pixel Buffer Object download failed!\n"); 
                }
                glBindBuffer(GL_ARRAY_BUFFER, 0);
            ...
            {
                CopyArrayToDevice(cqCommandQueue, m_dPos[m_currentRead], data, m_numBodies);
            }
        }
            break;
        case BODYSYSTEM_VELOCITY:
            CopyArrayToDevice(cqCommandQueue, m_dVel[m_currentRead], data, m_numBodies);
            break;
    }       
}
