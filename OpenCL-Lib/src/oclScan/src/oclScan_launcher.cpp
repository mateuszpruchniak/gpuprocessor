...
extern "C" void initScan(cl_context cxGPUContext, cl_command_queue cqParamCommandQue, const char **argv){
    ...
    shrLog(LOGBOTH, 0.0, "...checking minimum supported workgroup size\n");
        ...
        if( (szScanExclusiveLocal1 < WORKGROUP_SIZE) || (szScanExclusiveLocal2 < WORKGROUP_SIZE) || (szUniformUpdate < WORKGROUP_SIZE) ){
            shrLog(LOGBOTH, 0, "ERROR: Minimum work-group size %u required by this application is not supported on this device.\n", WORKGROUP_SIZE);
            exit(0);
        }

    shrLog(LOGBOTH, 0, "...allocating internal buffers\n");
...
extern "C" size_t scanExclusiveShort(
    ...
    shrCheckError( factorizationRemainder == 1, shrTRUE);

    //Check supported size range
    shrCheckError( (arrayLength >= MIN_SHORT_ARRAY_SIZE) && (arrayLength <= MAX_SHORT_ARRAY_SIZE), shrTRUE );

    //Check total batch size limit
    shrCheckError( (batchSize * arrayLength) <= MAX_BATCH_ELEMENTS, shrTRUE );

    //Check all work-groups to be fully packed with data
    shrCheckError( (batchSize * arrayLength) % (4 * WORKGROUP_SIZE) == 0, shrTRUE);
...
extern "C" size_t scanExclusiveLarge(
    ...
    //Check supported size range
    shrCheckError( (arrayLength >= MIN_LARGE_ARRAY_SIZE) && (arrayLength <= MAX_LARGE_ARRAY_SIZE), shrTRUE );
