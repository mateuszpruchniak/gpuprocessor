...
void ParticleSystem::_initialize(int numParticles){
    ...
    //Allocate GPU data
    shrLog(LOGBOTH, 0, "Allocating GPU Data buffers...\n\n");
...
void ParticleSystem::update(float deltaTime){
...
#if(1)
    ////copyArrayFromDevice(m_hPos,          m_dPos,          0, m_numParticles * sizeof(float) * 4);
    ////copyArrayFromDevice(m_hVel,          m_dVel,          0, m_numParticles * sizeof(float) * 4);
    ////copyArrayFromDevice(m_hReorderedPos, m_dReorderedPos, 0, m_numParticles * sizeof(float) * 4);
    ////copyArrayFromDevice(m_hReorderedVel, m_dReorderedVel, 0, m_numParticles * sizeof(float) * 4);
    ////copyArrayFromDevice(m_hHash,         m_dHash,         0, m_numParticles * sizeof(uint));
    ////copyArrayFromDevice(m_hIndex,        m_dIndex,        0, m_numParticles * sizeof(uint));
    ////copyArrayFromDevice(m_hCellStart,    m_dCellStart,    0, m_numGridCells * sizeof(uint));
    ////copyArrayFromDevice(m_hCellEnd,      m_dCellEnd,      0, m_numGridCells * sizeof(uint));

    ////for(unsigned int i = 0; i < 4 * m_numParticles; i++){
    ////    if(isNan(m_hReorderedPos[i]))
    ////        shrLog(LOGBOTH, 0, "NAN in reorderedPos at %i\n", i);
    ////    if(isNan(m_hReorderedVel[i]))
    ////        shrLog(LOGBOTH, 0, "NAN in reorderedVel at %i\n", i);
    ////}

    ////for(unsigned int i = 0; i < m_numParticles; i++)
    ////    if(m_hIndex[i] >= m_numParticles){
    ////        shrLog(LOGBOTH, 0, "Index out of range: %u at %u\n", m_hIndex[i], i);
    ////        break;
    ////    }

    ////for(unsigned int i = 0; i < m_numParticles; i++)
    ////    if(m_hHash[i] >= m_numGridCells){
    ////        shrLog(LOGBOTH, 0, "Hash out of range: %u at %u\n", m_hHash[i], i);
    ////        break;
    ////    }
    ...
    ////    if(m_hCellStart[i] >= m_numParticles){
    ////        shrLog(LOGBOTH, 0, "cellStart inconsistency: %u at %u\n", m_hCellStart[i], i);
    ////        break;
    ////    }

    ////    if(m_hCellEnd[i] < m_hCellStart[i]){
    ////        shrLog(LOGBOTH, 0, "void cell: %u, %u at %u\n", m_hCellStart[i], m_hCellEnd[i], i);
    ////        break;
    ////    }

    ////    int delta = m_hCellEnd[i] - m_hCellStart[i];
    ////    if(delta > maxdelta) maxdelta = delta;
    ////    if(delta < mindelta) mindelta = delta;
    ////};
    ////shrLog(LOGBOTH, 0, "(max, min) particles per cell: (%i, %i)\n", maxdelta, mindelta);
    ...
    collideHost(
        (memHandle_t)m_hVel,
        (memHandle_t)m_hReorderedPos,
        (memHandle_t)m_hReorderedVel,
        (memHandle_t)m_hIndex,
        (memHandle_t)m_hCellStart,
        (memHandle_t)m_hCellEnd,
        m_numParticles,
        m_numGridCells
    );
