...
float timestep = 0.4f;              // time slice for re-computation iteration
...
int main(int argc, const char** argv) 
{
    ...
    shrLog(LOGBOTH, 0, " grid: %d x %d x %d = %d cells\n", gridSize.x, gridSize.y, gridSize.z, gridSize.x * gridSize.y * gridSize.z);
    shrLog(LOGBOTH, 0, " particles: %d\n\n", numParticles);
...
{  
    ...
    {
        shrLog(LOGBOTH, 0, "Required OpenGL extensions missing !!!\n");
        Cleanup(EXIT_FAILURE);
    }
...
void TestNoGL()
{
    ...
    shrLog(LOGBOTH | MASTER, 0, "oclParticles, Throughput = %.4f, Time = %.5f, Size = %u, NumDevsUsed = %u\n", 
           (1.0e-6 * numParticles)/dAvgTime, dAvgTime, numParticles, 1); 
...
void Cleanup(int iExitCode)
{
    ...
    {
        shrLog(LOGBOTH | CLOSELOG, 0, "oclParticles.exe Exiting...\n");
    }
    else 
    {
        shrLog(LOGBOTH | CLOSELOG, 0, "oclParticles.exe Exiting...\nPress <Enter> to Quit\n");
