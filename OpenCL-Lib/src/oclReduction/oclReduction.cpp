...
template <class T>
void runTest( int argc, const char** argv, ReduceType datatype);
...
int main( int argc, const char** argv) 
{
    ...
    shrLog(LOGBOTH, 0, "Reducing array of type %s.\n", typeChoice);
                  ...
                  cl_mem d_odata)
{
    ...
    //shrLog(LOGBOTH, 0, "Profile Kernel %d\n", whichKernel);
    ...
    for (int i = 0; i < testIterations; ++i)
    {
        ...
        if (cpuFinalReduction)
        {
            ...
            clEnqueueReadBuffer(cqCommandQueue, d_odata, CL_FALSE, 0, numBlocks*sizeof(T), 
                                h_odata, 0, NULL, NULL);
        ...
        else
        {
            ...
            {
                // copy result from device to host
                clEnqueueReadBuffer(cqCommandQueue, d_odata, CL_FALSE, 0, s*sizeof(T), 
                                    h_odata, 0, NULL, NULL);
    ...
    if (needReadBack)
    {
        ...
        clEnqueueReadBuffer(cqCommandQueue, d_odata, CL_FALSE, 0, sizeof(T), 
...
{ 
    ...
    // print headers
    shrLog(LOGBOTH, 0, "Time in seconds for various numbers of elements for each kernel\n");
    shrLog(LOGBOTH, 0, "\n\n");
    shrLog(LOGBOTH, 0, "Kernel");
    for (int i = minN; i <= maxN; i *= 2)
    {
        shrLog(LOGBOTH, 0, ", %d", i);
    }
   
    for (int kernel = 0; kernel < 7; kernel++)
    {
        shrLog(LOGBOTH, 0, "\n");
        shrLog(LOGBOTH, 0, "%d", kernel);
        for (int i = minN; i <= maxN; i *= 2)
        {
            int numBlocks = 0;
            int numThreads = 0;
            ...
            shrLog(LOGBOTH, 0, ", %.4f m", reduceTime);
        }
    }
...
runTest( int argc, const char** argv, ReduceType datatype) 
{
    ...
    shrLog(LOGBOTH, 0, " %d elements\n", size);
    shrLog(LOGBOTH, 0, " %d threads (max)\n", maxThreads);
...
#endif
    {
        ...
        shrLog(LOGBOTH | MASTER, 0, "oclReduction, Throughput = %.4f, Time = %.5f, Size = %u, NumDevsUsed = %d\n", 
                1.0e-9 * (size * sizeof(int))/reduceTime, reduceTime, size, 1);
#endif
        ...
        if (datatype == REDUCE_INT)
        {
            shrLog(LOGBOTH, 0, " GPU result = %d\n", gpu_result);
            shrLog(LOGBOTH, 0, " CPU result = %d\n\n", cpu_result);

            shrLog(LOGBOTH, 0, "TEST %s\n\n", (gpu_result == cpu_result) ? "PASSED" : "FAILED");
        }
        else
        {
            shrLog(LOGBOTH, 0, " GPU result = %0.12f\n", gpu_result);
            shrLog(LOGBOTH, 0, " CPU result = %0.12f\n\n", cpu_result);
