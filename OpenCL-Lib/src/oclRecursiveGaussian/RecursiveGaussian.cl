...
// Defines for recursive gaussian demo
//*****************************************************************************
#define BLOCK_DIM 16

// 0 gives "fade-from-dark" edges from the model where zero value pixels are being averaged in
// 1 gives "extrapolation" edges from the model where pixels within radius are copied from the last value outside radius
                        ...
                        __local unsigned int* uiLocalBuff)
{
    ...
    {
        uiLocalBuff[get_local_id(1)*(BLOCK_DIM + 1)+get_local_id(0)] = uiDataIn[(yIndex * iWidth) + xIndex];
    }
    ...
    xIndex = mul24((int)get_group_id(1), BLOCK_DIM) + get_local_id(0);
    yIndex = mul24((int)get_group_id(0), BLOCK_DIM) + get_local_id(1);
    if((xIndex < iHeight) && (yIndex < iWidth))
    {
        uiDataOut[(yIndex * iHeight) + xIndex] = uiLocalBuff[get_local_id(0)*(BLOCK_DIM + 1)+get_local_id(1)];
    }
}
