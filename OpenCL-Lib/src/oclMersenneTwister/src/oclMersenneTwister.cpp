...
int main(int argc, const char **argv)
{
    ...
    shrLog(LOGBOTH, 0, "Initialization: load MT parameters and init host buffers...\n");
    mt_struct_stripped *h_MT = (mt_struct_stripped*)malloc(sizeof(mt_struct_stripped)*MT_RNG_COUNT); // MT para
    loadMTGPU(shrFindFilePath("MersenneTwister.dat", argv[0]), seed, h_MT, MT_RNG_COUNT);
    initMTRef(shrFindFilePath("MersenneTwister.raw", argv[0]));
    ...
    shrLog(LOGBOTH, 0, "Create and build program from %s...\n", clSourcefile);
    size_t szKernelLength; // Byte size of kernel code
    char *cMersenneTwister = oclLoadProgSource(shrFindFilePath(clSourcefile, argv[0]), "// My comment\n", &szKernelLength);
    shrCheckErrorEX(cMersenneTwister == NULL, false, NULL);
    ...
    shrLog(LOGBOTH, 0, "Call Mersenne Twister kernel on GPU...\n\n"); 
    ...
    #ifdef DO_BOXMULLER
        shrLog(LOGBOTH, 0, "Applying Box-Muller transformation on GPU.\n");  
        for (int iDevice = 0; iDevice < nDevice; i++)
        {
    ...
    shrLog(LOGBOTH | MASTER, 0, "oclMersenneTwister, Throughput = %.4f, Time = %.5f, Size = %u, NumDevsUsed = %u, Workgroup = %u\n", 
    ...
    shrLog(LOGBOTH, 0, "Compute CPU reference solution...\n");
    for (int iDevice = 0; iDevice < nDevice; iDevice++)
    {
    ...
    shrLog(LOGBOTH, 0, "Compare CPU and GPU results...\n");
    double sum_delta = 0;
    double sum_ref   = 0;
    for (int iDevice = 0; iDevice < nDevice; iDevice++)
    {
