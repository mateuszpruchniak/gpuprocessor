...
#ifdef _WIN32
    #pragma message ("Note: including stdio.h")
    #pragma message ("Note: including math.h")
    #pragma message ("Note: including rendercheckGL.h")
#endif

// Defines and enum for use with logging functions
// *********************************************************************
#define DEFAULTLOGFILE "SdkLogfileOpenCL.txt"
#define MASTERLOGFILE "SdkMasterLogOpenCL.csv"
extern "C" enum LOGMODES 
{
    ...
    MASTER =     8, // bit to signal "pre-pend standard Delta Time info"
...
// (Companion Inline Function lower on page)
#define shrEXIT(a, b) __shrEXIT(a, b)
...
//! Examples: shrLog(LOGBOTH , 0, "Function A\n"); 
...
// *********************************************************************
extern "C" int shrLog(int iLogMode, int iErrNum, const char* cFormatString, ...);
...
// Helper function to check diffs between 2 result arrays and count/report the errors
// *********************************************************************
extern "C" int shrDiffArray(const float* pfResult1, const float* pfResult2, int iNumElements);
...
////////////////////////////////////////////////////////////////////////////
extern "C" shrBOOL shrLoadPPM4ub(const char* file, unsigned char** data, 
...
////////////////////////////////////////////////////////////////////////////////
extern "C" shrBOOL shrComparePPM( const char *src_file, const char *ref_file, const float epsilon, const float threshold, bool verboseErrors = false );
...
inline void __shrCheckErrorEX(int iSample, int iReference, void (*pCleanup)(int), const char* cFile, const int iLine)
{
    ...
    {
        shrLog(LOGBOTH | ERRORMSG, iSample, "line %i , in file %s !!!\n\n" , iLine, cFile); 
        if (pCleanup != NULL)
        {
        ...
        {
            shrLog(LOGBOTH | CLOSELOG, 0, "Exiting...\n");
            exit(EXIT_FAILURE);
        }
    }
}
...
// Standardized Exit
// *********************************************************************
inline void __shrEXIT(int argc, const char** argv)
{
    if (!shrCheckCmdLineFlag(argc, argv, "noprompt")) 
    {        
        shrLog(LOGBOTH | CLOSELOG, 0, "\nPress <Enter> to Quit...\n");                  
        getchar();                                                           
    }       
    else 
    {
        shrLog(LOGBOTH | CLOSELOG, 0, "%s Exiting...\n", argv[0]); 
    }
