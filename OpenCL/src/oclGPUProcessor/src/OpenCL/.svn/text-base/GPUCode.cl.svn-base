...
void GetData(__global uchar* dataIn, __local uchar* dataOut, int iDevGMEMOffset, int iLocalPixOffset)
{
...
void SetZERO(__local uchar* dataOut, int iLocalPixOffset)
{
...
uchar4 GetDataFromLocalMemory( __local uchar* data,  int iLocalPixOffset )
{
	uchar4 pix;
...
void setData(__global char* data, char x , char y, char z, int iDevGMEMOffset )
{
...
uchar4 GetDataFromGlobalMemory( __global uchar* data,  int iDevGMEMOffset )
{
	uchar4 pix;
                      ...
                      unsigned int uiImageWidth, unsigned int uiDevImageHeight)
{
	    ...
	    { 
			GetData(ucSource,ucLocalData,iDevGMEMOffset,iLocalPixOffset);
	    }
	    else 
	    {
			SetZERO(ucLocalData,iLocalPixOffset);
	    }
	    ...
	    if (get_local_id(1) < 2)
	    {
			...
			if (((iDevYPrime + get_local_size(1)) < uiDevImageHeight) && (iImagePosX < uiImageWidth))
			{
				...
				GetData(ucSource,ucLocalData,iDevGMEMOffset + mul24(get_local_size(1), get_global_size(0)),iLocalPixOffset);
			}
			else 
			{
				SetZERO(ucLocalData,iLocalPixOffset);
	    ...
	    if (get_local_id(0) == (get_local_size(0) - 1))
	    {
			...
			if ((iDevYPrime >= 0) && (iDevYPrime < uiDevImageHeight) && (get_group_id(0) > 0))
			{
				...
				GetData(ucSource,ucLocalData,mul24(iDevYPrime, (int)get_global_size(0)) + mul24(get_group_id(0), get_local_size(0)) - 1,iLocalPixOffset);
			}
			else 
			{
				SetZERO(ucLocalData,iLocalPixOffset);
			...
			if (get_local_id(1) < 2)
			{
				...
				if (((iDevYPrime + get_local_size(1)) < uiDevImageHeight) && (get_group_id(0) > 0))
				{
					...
					GetData(ucSource,ucLocalData,mul24((iDevYPrime + (int)get_local_size(1)), (int)get_global_size(0)) + mul24(get_group_id(0), get_local_size(0)) - 1,iLocalPixOffset);
				}
				else 
				{
					SetZERO(ucLocalData,iLocalPixOffset);
	    ...
	    else if (get_local_id(0) == 0) // Work items with x ID at left workgroup edge will read right apron pixel
	    {
			...
			if ((iDevYPrime >= 0) && (iDevYPrime < uiDevImageHeight) && (mul24(((int)get_group_id(0) + 1), (int)get_local_size(0)) < uiImageWidth))
			{
				...
				GetData(ucSource,ucLocalData,mul24(iDevYPrime, (int)get_global_size(0)) + mul24((get_group_id(0) + 1), get_local_size(0)),iLocalPixOffset);
			}
			else 
			{
				SetZERO(ucLocalData,iLocalPixOffset);
			...
			if (get_local_id(1) < 2)
			{
				...
				if (((iDevYPrime + get_local_size(1)) < uiDevImageHeight) && (mul24((get_group_id(0) + 1), get_local_size(0)) < uiImageWidth) )
				{
					...
					GetData(ucSource,ucLocalData,mul24((iDevYPrime + (int)get_local_size(1)), (int)get_global_size(0)) + mul24((get_group_id(0) + 1), get_local_size(0)),iLocalPixOffset);
				}
				else 
				{
					SetZERO(ucLocalData,iLocalPixOffset);
