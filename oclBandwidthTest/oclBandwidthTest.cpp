...
int runTest(const int argc, const char **argv)
{
    ...
    if(shrGetCmdLineArgumentstr(argc, argv, "memory", &memModeStr))
    {
        ...
        {
            shrLog(LOGBOTH, 0, "Invalid memory mode - valid modes are pageable or pinned\n");
            shrLog(LOGBOTH, 0, "See --help for more information\n");
            return -1000;
        }
    }
    else
    ...
    if(shrGetCmdLineArgumentstr(argc, argv, "access", &memModeStr))
    {
        ...
        {
            shrLog(LOGBOTH, 0, "Invalid access mode - valid modes are direct or mapped\n");
            shrLog(LOGBOTH, 0, "See --help for more information\n");
            return -2000;
        }
    }
    ...
    if(shrGetCmdLineArgumentstr(argc, argv, "mode", &modeStr))
    {
        ...
        {
            shrLog(LOGBOTH, 0, " Quick Mode\n\n");
            mode = QUICK_MODE;
        }
        else if(strcmp(modeStr, "shmoo") == 0)
        {
            shrLog(LOGBOTH, 0, " Shmoo Mode\n\n");
            mode = SHMOO_MODE;
        }
        else if(strcmp(modeStr, "range") == 0)
        {
            shrLog(LOGBOTH, 0, " Range Mode\n\n");
            mode = RANGE_MODE;
        }
        else
        {
            shrLog(LOGBOTH, 0, "Invalid mode - valid modes are quick, range, or shmoo\n");
            shrLog(LOGBOTH, 0, "See --help for more information\n\n");
            return -3000;
        }
    }
    ...
    if(RANGE_MODE == mode)
    {
        if(shrGetCmdLineArgumenti( argc, argv, "start", &start))
        {
            if( start <= 0 )
            {
                shrLog(LOGBOTH, 0, "Illegal argument - start must be greater than zero\n");
                return -4000;
            }   
        }
        else
        {
            shrLog(LOGBOTH, 0, "Must specify a starting size in range mode\n");
            shrLog(LOGBOTH, 0, "See --help for more information\n");
            return -5000;
        }
        ...
        if(shrGetCmdLineArgumenti( argc, argv, "end", &end))
        {
            if(end <= 0)
            {
                shrLog(LOGBOTH, 0, "Illegal argument - end must be greater than zero\n");
                return -6000;
            }

            if(start > end)
            {
                shrLog(LOGBOTH, 0, "Illegal argument - start is greater than end\n");
                return -7000;
            }
        }
        else
        {
            shrLog(LOGBOTH, 0, "Must specify an end size in range mode.\n");
            shrLog(LOGBOTH, 0, "See --help for more information\n");
            return -8000;
        }
        ...
        if(shrGetCmdLineArgumenti( argc, argv, "increment", &increment))
        {
            if(increment <= 0)
            {
                shrLog(LOGBOTH, 0, "Illegal argument - increment must be greater than zero\n");
                return -9000;
            }
        }
        else
        {
            shrLog(LOGBOTH, 0, "Must specify an increment in user mode\n");
            shrLog(LOGBOTH, 0, "See --help for more information\n");
            return -10000;
        }
    }
    ...
    cxGPUContext = clCreateContextFromType(0, CL_DEVICE_TYPE_GPU, NULL, NULL, NULL);
    if (cxGPUContext == (cl_context)0) 
    {
        shrLog(LOGBOTH, 0, "Failed to create OpenCL context!\n");
        return -11000;    
    }
...
void printResultsReadable(unsigned int *memSizes, double* bandwidths, unsigned int count, memcpyKind kind, accessMode accMode, memoryMode memMode, int iNumDevs)
{
    ...
    {
        shrLog(LOGBOTH, 0, " Device to Device Bandwidth, %i Device(s)\n", iNumDevs);
    }
    else 
    {
        if (kind == DEVICE_TO_HOST)
        {
            shrLog(LOGBOTH, 0, " Device to Host Bandwidth, %i Device(s), ", iNumDevs);
        }
        else if (kind == HOST_TO_DEVICE)
        {
            shrLog(LOGBOTH, 0, " Host to Device Bandwidth, %i Device(s), ", iNumDevs);
        }
        if(memMode == PAGEABLE)
        {
            shrLog(LOGBOTH, 0, "Paged memory");
        }
        else if (memMode == PINNED)
        {
            shrLog(LOGBOTH, 0, "Pinned memory");
        }
        if(accMode == DIRECT)
        {
            shrLog(LOGBOTH, 0, ", direct access\n");
        }
        else if (accMode == MAPPED)
        {
            shrLog(LOGBOTH, 0, ", mapped access\n");
        }
    }

    shrLog(LOGBOTH, 0, "   Transfer Size (Bytes)\tBandwidth(MB/s)\n");
    unsigned int i; 
    for(i = 0; i < (count - 1); i++)
    {
        shrLog(LOGBOTH, 0, "   %u\t\t\t%s%.1f\n", memSizes[i], (memSizes[i] < 10000)? "\t" : "", bandwidths[i]);
    }
    shrLog(LOGBOTH, 0, "   %u\t\t\t%s%.1f\n\n", memSizes[i], (memSizes[i] < 10000)? "\t" : "", bandwidths[i]);
}
...
void printResultsCSV(unsigned int *memSizes, double* bandwidths, unsigned int count, memcpyKind kind, accessMode accMode, memoryMode memMode, int iNumDevs)
{
    ...
    // log config information 
        ...
        if (kind == DEVICE_TO_DEVICE)
        {
            sprintf_s(cConfig, 256, "D2D");
        }
        else 
        {
            if (kind == DEVICE_TO_HOST)
            {
                sprintf_s(cConfig, 256, "D2H");
            }
            else if (kind == HOST_TO_DEVICE)
            {
                sprintf_s(cConfig, 256, "H2D");
            }

            if(memMode == PAGEABLE)
            {
                strcat_s (cConfig, 256, "-Paged");
            }
            else if (memMode == PINNED)
            {
                strcat_s(cConfig, 256, "-Pinned");
            }

            if(accMode == DIRECT)
            {
                strcat_s(cConfig, 256, "-Direct");
            }
            else if (accMode == MAPPED)
            {
                strcat_s(cConfig, 256, "-Mapped");
            }
        }
    #else
        // log config information 
        ...
        else 
        {
            ...
            if(memMode == PAGEABLE)
            {
                strcat(cConfig, "-Paged");
            }
            else if (memMode == PINNED)
            {
                strcat (cConfig, "-Pinned");
            }

            if(accMode == DIRECT)
            {
                strcat (cConfig, "-Direct");
            }
            else if (accMode == MAPPED)
            {
                strcat (cConfig, "-Mapped");
            }
        }    
    #endif
    ...
    for(i = 0; i < count; i++)
    {
        ...
        shrLog(LOGBOTH | MASTER, 0, "oclBandwidthTest-%s, Bandwidth = %.1f, Time = %.5f, Size = %u, NumDevsUsed = %i\n", cConfig, bandwidths[i], dSeconds, memSizes[i], iNumDevs);
    }
}
...
void printHelp(void)
{
    shrLog(LOGBOTH, 0, "Usage:  bandwidthTest [OPTION]...\n");
    shrLog(LOGBOTH, 0, "Test the bandwidth for device to host, host to device, and device to device transfers\n");
    shrLog(LOGBOTH, 0, "\n");
    shrLog(LOGBOTH, 0, "Example:  measure the bandwidth of device to host pinned memory copies in the range 1024 Bytes to 102400 Bytes in 1024 Byte increments\n");
    shrLog(LOGBOTH, 0, "./bandwidthTest --memory=pinned --mode=range --start=1024 --end=102400 --increment=1024 --dtoh\n");

    shrLog(LOGBOTH, 0, "\n");
    shrLog(LOGBOTH, 0, "Options:\n");
    shrLog(LOGBOTH, 0, "--help\tDisplay this help menu\n");
    shrLog(LOGBOTH, 0, "--csv\tPrint results as a CSV\n");
    shrLog(LOGBOTH, 0, "--device=[deviceno]\tSpecify the device device to be used\n");
    shrLog(LOGBOTH, 0, "  all - compute cumulative bandwidth on all the devices\n");
    shrLog(LOGBOTH, 0, "  0,1,2,...,n - Specify any particular device to be used\n");
    shrLog(LOGBOTH, 0, "--access=[ACCESSMODE]\tSpecify which memory access mode to use\n");
    shrLog(LOGBOTH, 0, "  direct   - direct device memory\n");
    shrLog(LOGBOTH, 0, "  mapped   - mapped device memory\n");
    shrLog(LOGBOTH, 0, "--memory=[MEMMODE]\tSpecify which memory mode to use\n");
    shrLog(LOGBOTH, 0, "  pagable   - pageble memory\n");
    shrLog(LOGBOTH, 0, "  pinned    - pinned memory\n");
    shrLog(LOGBOTH, 0, "--mode=[MODE]\tSpecify the mode to use\n");
    shrLog(LOGBOTH, 0, "  quick - performs a quick measurement\n");
    shrLog(LOGBOTH, 0, "  range - measures a user-specified range of values\n");
    shrLog(LOGBOTH, 0, "  shmoo - performs an intense shmoo of a large range of values\n");

    shrLog(LOGBOTH, 0, "--htod\tMeasure host to device transfers\n");   
    shrLog(LOGBOTH, 0, "--dtoh\tMeasure device to host transfers\n");
    shrLog(LOGBOTH, 0, "--dtod\tMeasure device to device transfers\n");
    
    shrLog(LOGBOTH, 0, "Range mode options\n");
    shrLog(LOGBOTH, 0, "--start=[SIZE]\tStarting transfer size in bytes\n");
    shrLog(LOGBOTH, 0, "--end=[SIZE]\tEnding transfer size in bytes\n");
    shrLog(LOGBOTH, 0, "--increment=[SIZE]\tIncrement size in bytes\n");
}
